...
period_begin <- as.Date("2021-06-01")
period_end <- as.Date("2020-06-01")
...
drupal_all_logins <- as.integer(sum(login$`Logins during time period`))
...
if (file.exists("deposits.rds")) {
  ...
  # And unnest the ugly json arrays as a tibble
  ...
  # Some basic formatting, e.g. truncate names
  ...
  # Easier to save all these calls as one list
...
im_drupal %>% filter(!is.na(member)) %>%
  replace_na(replace = list(created_since=0, logins=0, n=0)) %>%
...
```{r views}
# Turn these data into a mts object
# This can be plot like a ts
