...
if (file.exists("deposits.rds")) {
  ...
  # Detect PCRC on the basis of the publisher field
  deposits$member[deposits$publisher=="Palliative Care Research Cooperative QDR-EOLPC "] <-
    "PCRC"
...
im_drupal <- login %>% group_by(member, .drop=FALSE) %>% filter(Created < period_end & Created > period_begin) %>% 
  summarize(created_since=length(`Name/Email`)) %>% full_join(im_drupal) %>%
