...
```{r members_drupal}
members_drupal <- count(group_by(login, member, .drop=FALSE))

members_drupal <- login %>% group_by(member, .drop=FALSE) %>% summarize(recent=sum(recent)) %>% full_join(members_drupal)
